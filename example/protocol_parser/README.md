...
This example shows how to integrate fuzzmate into an existing CMake project. It
is a small packet decoder/validator with global configuration state that
fuzzmate samples automatically.
...
(`static`), so fuzzmate mangles their symbols with `objcopy` to make them
...
1. **fuzzmate** (aro parsing) — so it sees the correct struct layout
...
- **fuzzmate** built and installed (`zig build` in the repo root)
- **CMake** >= 3.20
...
The key is `fuzzmate_add_fuzzer()` from `find_package(Fuzzmate)`:
...
`PUBLIC`, they propagate automatically to every stage of the fuzzmate pipeline —
no need to duplicate them. This single call creates a CMake target that:

1. **Runs fuzzmate** on the target `.c` files to generate `fuzzer.c`,
